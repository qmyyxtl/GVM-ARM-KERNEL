...
static ssize_t tso_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
	...
	s = snprintf(str, sizeof(str), "Boot State: %d (%s)\n"
						...
						"0 (%s)\t1 (%s)\t3 (%s)\n"
						"4 (%s)\t5 (%s)\t7 (%s)\n",
						tso_state, tso_hints[tso_state], state,
						tso_hints[state], tso_hints[0], tso_hints[1],
						tso_hints[3], tso_hints[4], tso_hints[5],
						tso_hints[7]);
...
static int __init arch_kdebugfs_init(void)
{
	...
	if (config & CPUCFG3_SFB) {
		debugfs_create_file("sfb_state", 0644,
	...
	if (config & (CPUCFG3_ALDORDER_CAP | CPUCFG3_ASTORDER_CAP)) {
		debugfs_create_file("tso_state", 0644,
			    arch_debugfs_dir, &tso_state, &tso_fops);
		tso_state = (csr_read32(LOONGARCH_CSR_IMPCTL1) &
						CSR_LDSTORDER_MASK) >> CSR_LDSTORDER_SHIFT;
	}
